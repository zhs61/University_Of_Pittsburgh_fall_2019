...
public class Evaluate {
    public static void main(String[] args) throws InterruptedException, IOException {
        ...
        Peer_server[] peers=new Peer_server[6];
            ...
            for (int i = 2; i <= peers.length; i++ ) {
            	...
            	result = testMultiThread(peersList, filenames, request_num);
     ...
     * @throws IOException 
     */
	private static ArrayList<double[]> testMultiThread(ArrayList<Peer_server> peersList, String[] filenames, int request_num) throws IOException, InterruptedException {
		int numOfPeer = peersList.size();
		Random r=new Random();
		...
		for (int i = 0; i < peersList.size(); i++) {
			...
			for (int j = 0; j < request_num; j++) {
				boolean existFile = peer.get_file(filename);
				File file = new File("file/peer" + (i+1) + "/" + filename);
				if (existFile) {
					file.delete();
				}
			}
